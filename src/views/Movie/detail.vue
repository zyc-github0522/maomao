    ...
    <div id="content" class="contentDetail" v-else>
      <div class="detail_list">
        <div
          ...
          :style="{
            'background-image':
              'url(' + detailMovei.img.replace(/w\.h/, '148.208') + ')',
          }"
        ></div>
        <div class="detail_list_filter"></div>
        <div class="detail_list_content">
          <div class="detail_list_img">
            <img :src="detailMovei.img | setWH('148.208')" />
          </div>
          <div class="detail_list_info">
            <h2>{{ detailMovei.nm }}</h2>
            <p>{{ detailMovei.enm }}</p>
            <p>{{ detailMovei.sc }}</p>
            <p>{{ detailMovei.cat }}</p>
            <p>{{ detailMovei.src }} / {{ detailMovei.dur }}分钟</p>
            <p>{{ detailMovei.pubDesc }}</p>
          </div>
        </div>
      </div>
      ...
      <div class="detail_player swiper-container" ref="detail_player">
        <ul class="swiper-wrapper">
          <li
            class="swiper-slide"
            v-for="(item, index) in detailMovei.photos"
            :key="index"
          >
            <div>
...
export default {
  ...
  mounted() {
    ...
    this.axios('/ajax/detailmovie?movieId=' + this.movieId).then((res) => {
      console.log(res.data.detailMovie)
      this.detailMovei = res.data.detailMovie
